...
body = cos.get_object(Bucket=cos_credentials['BUCKET'],Key='insurance3_processed.csv')['Body']
...
#define model
training_data = training_data.reshape(n_train,122,1)
test_data = test_data.reshape(n_rows-n_train,122,1)

inputs = Input(shape=training_data.shape[1:])
common_branch = LSTM(128, input_shape=training_data.shape[1:], activation='relu', return_sequences=True)(inputs)
common_branch = Dropout(0.2)(common_branch)
common_branch = LSTM(64, activation='relu')(common_branch)
common_branch = Dropout(0.2)(common_branch)
common_branch = Dense(32, activation='relu')(common_branch)
common_branch = Dropout(0.2)(common_branch)
common_branch = LSTM(128, activation='relu', input_shape=training_data.shape[1:], return_sequences=True)(inputs)
common_branch = Dropout(0.2)(common_branch)
common_branch = LSTM(64, activation='relu')(common_branch)
common_branch = Dropout(0.2)(common_branch)
common_branch = Dense(10, activation='relu')(common_branch)
...
history = model.fit(training_data, training_labels_dict, validation_data=(test_data, test_labels_dict), epochs=50)
...
try:
    SaveLoad.cos_save_weights(model, cos, cos_credentials, 'ins_rnn_weights.txt')
except:
    print('File upload failed')
    
#return predictions on the first elements of the dataset
pred_data = df.loc[0:10,'provincia_AG':'cilindrata'].values
pred_data = pred_data.reshape(11,122,1)

(inc, fur, esp, cri, evn, inf, ass, tut) = return_predictions(model.predict(pred_data))
